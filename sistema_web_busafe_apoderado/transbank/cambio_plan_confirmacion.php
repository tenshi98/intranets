...
if ($output->responseCode == 0) {
    ...
    */
	...
	if ( empty($error) ) {
		...
		$montoPago      = $_SESSION['plan']['PagoOnline'];
		$MontoPactado   = $_SESSION['plan']['MontoPactado'];
		...
		$query  = "UPDATE `apoderados_listado_planes_contratados` SET ".$a." WHERE idApoderado = '$idApoderado' AND idEstado='1'";
		//Consulta
		...
		// inserto los datos de registro en la db
		$query  = "INSERT INTO `vehiculos_facturacion_apoderados_pago` (idTipoPago, 
		nDocPago, fechaPago, DiaPago, idMesPago, AnoPago, montoPago, idUsuarioPago,
		...
		$_SESSION['usuario']['basic_data']['PlanNombre']         = $_SESSION['plan']['PlanNombre'];
		$_SESSION['usuario']['basic_data']['TipoPlanNombre']     = $_SESSION['plan']['TipoPlanNombre'];
		$_SESSION['usuario']['basic_data']['TipoPlan_idCobro']   = $idCobro;
		$_SESSION['usuario']['basic_data']['PlanValor_Mensual']  = $_SESSION['plan']['PlanValor_Mensual'];
		$_SESSION['usuario']['basic_data']['PlanValor_Anual']    = $_SESSION['plan']['PlanValor_Anual'];				
				
		/****************************************************************************************************/
		switch ($idCobro) {
			//Mensual
			case 1:
				$PlanValor = $_SESSION['plan']['PlanValor_Mensual'];
				break;
			//Anual
			case 2:
				$PlanValor = $_SESSION['plan']['PlanValor_Anual'];
				break;
		}
		switch ($idPlan) {
			//1 Hijo
		...
		$BodyMail .= '<strong>Plan:</strong> De '.$_SESSION['old_plan']['PlanNombre'].' a '.$_SESSION['plan']['PlanNombre'].'<br/>';
		$BodyMail .= '<strong>Modalidad de Pago:</strong> De '.$_SESSION['old_plan']['TipoPlanNombre'].' a '.$_SESSION['plan']['TipoPlanNombre'].'<br/>';
...
//error
	...
	//redirijo
	header( 'Location: ../principal_contratos.php?rechazado='.$OrdenCompra );
	die;
