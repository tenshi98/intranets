...
if(isset($sesion_IP_Client)&&$sesion_IP_Client!=''){
	...
	if($sesion_Activa!=0){
		...
		$rowUserId = db_select_nrows (false, 'idApoderado','apoderados_listado', '', 'Rut = "'.$sesion_usuario.'" AND password = "'.$sesion_password.'"', $dbConn, 'Ninguno', basename($_SERVER["REQUEST_URI"], ".php"), 'Ninguno');
	...
	//Si no hay sesion activa	
		...
		//se consulta la base de datos
		$rowUserIP = db_select_nrows (false, 'idIpUsuario','apoderados_listado_ip', '', 'IP_Client = "'.$sesion_IP_Client.'"', $dbConn, 'Ninguno', basename($_SERVER["REQUEST_URI"], ".php"), 'Ninguno');
